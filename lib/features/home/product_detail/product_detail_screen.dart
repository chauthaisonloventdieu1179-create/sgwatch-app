...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  void _selectVariant(GroupedProduct v, int index) {
    ...
    if (_variantStripController.hasClients) {
      _variantStripController.animateTo(
        targetOffset.clamp(
          0.0,
          _variantStripController.position.maxScrollExtent,
        ),
  ...
  Future<void> _onBuyTap() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_cartVM.error ?? 'Không thể thêm vào giỏ hàng.'),
        ),
      );
    }
  }
  ...
  Widget _buildVariantSection() {
    ...
    final selected = variants
        .where((v) => v.id == _viewModel.product.id)
        .firstOrNull;
  ...
  Future<void> _confirmDeleteReview(ReviewModel review) async {
    ...
    if (confirmed == true) {
      final success = await _viewModel.deleteReview(review.id);
      if (mounted && !success) {
        ScaffoldMessenger.of(context).showSnackBar(
